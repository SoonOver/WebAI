...
function formatPanelProgress(entry) {
  const index = Number(entry?.lastPanelIndex);
  const count = Number(entry?.lastPanelCount);
  if (!Number.isInteger(index) || !Number.isInteger(count) || count <= 1) return '';
  return `Panel ${Math.min(index + 1, count)} / ${count}`;
}
...
export default function HistoryScreen({ navigation }) {
  ...
  const renderItem = useCallback(
    ({ item }) => {
      const panelProgress = formatPanelProgress(item);
      const chapterLabel = panelProgress && item.lastChapter
        ? `${item.lastChapter} · ${panelProgress}`
        : item.lastChapter;
